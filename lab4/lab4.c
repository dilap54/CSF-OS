...
int main(){ 
    ...
    //Вызвать santa в отдельном потоке
    pthread_create(&santa_thread, NULL, santa, NULL);//Запуск потока санты (санта один, поток один)

    for (int i=0; i<M; i++){//Запуск потоков эльфов (их много, поэтому потоки создаются в цикле)
        //Создать поток эльфа
...
void *elf(int id){ //int id - номер эльфа. Нужно, чтобы в логах можно было различить эльфов
